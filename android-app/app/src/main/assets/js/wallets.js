...
function renderWallets() {
  ...
  MidoriState.wallets.forEach((wallet, index) => {
    ...
    const cardHTML = `
      <div class="wallet-card" style="background: linear-gradient(135deg, ${escapeHtml(wallet.color)} 0%, rgba(20,40,20,0.85) 100%);">
        <div class="wallet-card-header">
          <div>
            <div class="wallet-name-label">${escapeHtml(wallet.name)}</div>
            <div style="font-size: 10px; opacity:0.6; margin-top:2px;">${maskedCardNo}</div>
          </div>
          <span class="wallet-type-badge">${escapeHtml(wallet.type)}</span>
        </div>
        <div class="wallet-balance-display">${formattedBaseBalance}</div>
        <div class="wallet-card-footer">
          <span>Native: ${formattedNativeBalance}</span>
          <div style="display:flex; align-items:center; gap:8px;">
            <button class="wallet-edit-btn" data-action="openEditWalletModal" data-arg="${escapeHtml(wallet.id)}" title="Edit Wallet" style="background:none; border:none; color:white; opacity:0.8; cursor:pointer; display:inline-flex; align-items:center; padding:4px; transition: opacity 0.2s;">
            ...
            <button class="wallet-delete-btn" data-action="triggerWalletDelete" data-arg="${escapeHtml(wallet.id)}" title="Delete Wallet">
