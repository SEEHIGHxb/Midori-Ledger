...
function renderSchedules() {
  ...
  MidoriState.schedules.forEach(sched => {
    ...
    const walletName = escapeHtml(wallet ? wallet.name : 'Unknown');
    // Mirror scheduler.js: a schedule may carry its own currency.
    const currency = sched.currency || (wallet ? wallet.currency : 'JPY');
    ...
    const itemHTML = `
      <div class="schedule-item" style="border-left: 4px solid ${sched.active ? 'var(--green-matcha)' : 'var(--border-color)'};">
        <div class="schedule-meta">
          <span class="sched-frequency-badge">${escapeHtml(sched.frequency)}</span>
          <div class="sched-details">
            <span class="sched-title">${escapeHtml(sched.title)}</span>
        ...
        <div class="sched-finances">
          ...
          <div style="display:flex; align-items:center; gap:8px;">
            <button class="btn-secondary" data-action="toggleScheduleActive" data-arg="${escapeHtml(sched.id)}" style="padding:6px 12px; font-size:12px;${isExpired ? ' opacity:0.5; cursor:not-allowed;' : ''}" ${isExpired ? 'disabled' : ''}>
              ${sched.active ? 'Pause' : 'Resume'}
            </button>
            <button class="btn-secondary" data-action="openEditScheduleModal" data-arg="${escapeHtml(sched.id)}" title="Edit Schedule" style="padding:6px; display:inline-flex; align-items:center; justify-content:center;">
            ...
            <button class="btn-icon-danger" data-action="triggerScheduleDelete" data-arg="${escapeHtml(sched.id)}">
...
function openEditScheduleModal(schedId) {
  ...
  MidoriState.wallets.forEach(w => {
    editSchedWallet.insertAdjacentHTML('beforeend', `<option value="${escapeHtml(w.id)}">${escapeHtml(w.name)} (${escapeHtml(w.currency)})</option>`);
  });
