...
The webfonts follow the same rule. `fonts/inter-latin.woff2` and `fonts/outfit-latin.woff2` (Inter v20, Outfit v15, SIL OFL 1.1) are vendored and declared in `css/fonts.css`. They were loaded from Google Fonts, which meant a personal finance app announced every page load to a third party, and — being cross-origin — they were the one asset the service worker never cached, so typography broke offline and under the `file://` wrapper. Both are variable fonts covering the full weight range, so one file per family serves every weight (~80 KB total).

**When removing a remote asset, check for more than one reference.** The fonts were declared twice: a `<link>` in `index.html` *and* an `@import` at the top of `css/style.css`. Deleting the visible `<link>` left the `@import` fetching from the CDN, and grepping for the family name missed it because the two spell it differently (`Noto Serif JP` vs `Noto+Serif+JP`). The reliable check is to load the page and look at actual network requests; `tests/no-external-assets.test.js` now guards both forms plus the CSP allow-list.
...
It also carries three source-level guards for invariants that fail silently in the browser rather than throwing: no inline event-handler attributes (the CSP blocks them), the modals' dialog semantics and focus behaviour, and no remote asset references. These read the real files as text, so they hold without a DOM. Note that `tests/modal-a11y.test.js` cannot prove real-keypress Tab containment — synthetic `KeyboardEvent`s do not move focus — so verify Tab/Shift+Tab in a dialog by hand after touching that code.
