...
function updateVirtualDate(newDateStr) {
  ...
  if (newDateStr < oldDateStr) {
    // Tombstoned, or rolling the date back on one device would leave the other
    // device holding those future occurrences and push them straight back.
    recordDeletion(
      MidoriState.transactions
        .filter(tx => tx.scheduledId && tx.date > newDateStr)
        .map(tx => tx.id)
    );
